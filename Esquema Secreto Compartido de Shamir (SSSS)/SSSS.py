...
class ocultar:
    ...
    def get_shares(self,key):
        ...
        while True:
            try:
                total_shares = int(input("Ingresa el número total de claves a generar: "))
                if total_shares<0:
                    error(key)
                break    
            except ValueError:
        ...
        while True:
            try:
                req_shares = int(input("Ingresa el número requerido de claves para la desencriptación del archivo: "))
...
prueba = ocultar()
key = prueba.get_key()
shares = prueba.get_shares(key)
prueba.print_shares(shares)
prueba.encrypt(key)